...
## 2026-02-23
- **Fortify: gate-routing navigation for auto-patch**: Bot spawns inside the walled perimeter during `/bot fortify patch <name> auto`, but edge/tower approach positions are 3 blocks outside the wall. Baritone pathfinder treats walls as impassable, so the bot would walk into the wall and get stuck. Added `navigateThroughGateIfNeeded()`: before navigating to an exterior approach position, checks if bot is inside the convex hull via `pointInConvexHull()`. If inside and target is outside, routes through the gatehouse's 3-block open gap — walks to gate center (interior, unobstructed), then through the gap to an exit point 4 blocks along the outward normal. Gate routing fires once on the first edge, then skips for subsequent edges since the bot stays outside. Called from both `navigateToEdgeApproach()` and `executeTowerVertexWithRetries()`. Falls back to normal navigation if gate walk fails.
