...
## 2026-02-23
- **Fortify: fix break-through traversal, earlier scaffolding, step-onto-structure**: Three fixes to fortification building. (1) `tryBreakThroughObstacle` now uses direct movement impulse toward a point 2 blocks past the gap instead of `walkTowardBlock`, which would bail immediately due to its `distSq < 6.0` early-exit (target was only 1 block away). Eliminates 30+ futile break-replace cycles at the same position. (2) `ensureCanReachBlockWithEffort` now scaffolds on pass 1 when `verticalDiff >= 4` (clearly unreachable from ground — jumping only reaches +2.5). Still waits for pass 2 for marginal cases (verticalDiff 2-3). (3) `executeTowerScaffoldPhase` now tries stepping onto adjacent solid structure blocks from the scaffold to extend placement reach. Bot sneaks onto wall/tower blocks 1 block from scaffold, places newly-reachable blocks, then returns to scaffold for safe teardown.
