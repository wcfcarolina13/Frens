...
## 2026-02-22
- **Fortify: break-through stuck recovery**: When the bot gets stuck navigating between wall segments (wedged in corners between existing cobblestone walls), it now mines through one blocking block as a last resort, walks through the gap, then best-effort replaces the block. Safety predicate prevents mining layout blocks, village structures, containers, doors, beds, fences, and blocks adjacent to buildings. Max one break-through per `walkToTarget()` call to prevent tunnel mining. Integrated into `walkToTarget()` (stuck handler), `tryUnwedgeFromTightSpace()` (failure path), and `navigateToEdgeApproach()` (post-MovementService fallback).
