...
## 2026-02-23
- **Fortify: gate-routing v2 — multi-hop nav + verified exit**: Fixed gate routing failures from v1. Root cause: on large hulls (144x167 blocks), the gate center was 78+ blocks from the bot — a single pathfind+walkToTarget(10s) only covered ~55 blocks, leaving the bot stranded mid-hull. Also, the exit point 4 blocks along the outward normal landed right on the hull boundary (cross product = -1). Fixes: (1) Multi-hop navigation to gate center — up to 3 pathfind+walk cycles with progress checking, aborts if a hop makes no progress. (2) Exit point verification loop — starts at 6 blocks along the outward normal and extends by 2 up to 20 until `pointInConvexHull()` confirms the point is outside. (3) Post-walk retry — if still inside after the gate walk, extends 8 more blocks along the normal for a second attempt. (4) Bail-out at 10+ blocks from gate center to avoid wasting time when interior pathing fails.
