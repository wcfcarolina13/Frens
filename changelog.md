...
## 2026-02-23
- **Fortify: gate-routing v3 — interior approach at surface level**: Fixed the real root cause of gate routing failures: bot was arriving at the gate center at moat level (Y=-60) instead of surface level (Y=-58). The moat is 3 blocks deep on the exterior side of the wall, and the gate gap is only walkable at surface level. The bot fell into the moat approaching the gate, then couldn't walk through because wall foundations blocked at moat level. Fix: added an interior approach waypoint 3 blocks INWARD from gate center along the negative normal. This point has no moat (moat is exterior-only), so the bot arrives at correct surface Y. Three-phase walk: (1) multi-hop pathfind to interior approach point at surface level, (2) walk to gate center through the 3-block gap, (3) walk to exit through moat bridge. Also increased exit verification distance (start at 6, extend to 20) and retry extension (10 blocks) for large hulls.
