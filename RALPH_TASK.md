...
- [ ] **Dangerous-pursuit gate**: Bot chases mobs / item drops / XP orbs into hazardous situations (dark caves full of mobs, fall pits). Composable rules to reject pursuit:
  - Predicted fall damage > N hp on the route → reject.
  - Light level 0 at target → reject.
  - Multiple hostile mobs visible at target → reject.
  - Non-aggro'd mob → only pursue if bot has a long-range weapon (bow / loaded crossbow). Otherwise leave them alone.
  - Apply gate at the same call site as drop-sweep target acceptance and combat target acceptance — share the predicate. Likely lives next to [DropSweeper](src/main/java/net/wcfcarolina13/GameAI/DropSweeper.java) target filter and the engagement filter in [BotEventHandler.java:3657](src/main/java/net/wcfcarolina13/GameAI/BotEventHandler.java#L3657).
- [ ] **Locked-gate enclosure respect (drop-sweep + pursuit)**: Admin-marked locked gates already block pathfinding ([LockableBlockService](src/main/java/net/wcfcarolina13/GameAI/services/LockableBlockService.java) + memory: "Lockable blocks"), but the bot still tries to chase drops/XP *into* the enclosure, gets stuck against the wall, and lets mobs escape past it. Fix: drop-sweep + pursuit target filters need to reject any candidate whose position is on the far side of a locked-gate enclosure boundary relative to the bot. Reuse existing `LockableBlockService` zone semantics — don't add new walls.
- [ ] **Bed selection bugs (two related)**:
  - (a) Bot sleeps in the *commander's* bed instead of its own. `SleepSkill` / sleep-bed-finding heuristic must prefer beds owned by / assigned to / nearest-to the bot itself, not the closest unoccupied bed. Possibly tag beds the user has slept in as user-claimed, or check distance from bot's home/base anchor.
  - (b) When the user goes to sleep and only one nearby bed exists (the user's), and the bot has a bed in inventory, the bot should *place* its own bed somewhere reasonable (next to commander's, in the bot's claimed sleeping area, or anywhere it can place + reach) instead of giving up.
- [ ] **Pathfinding cache learning isn't measurably improving**: Despite the cache-learning system, in-game feel is that the bot doesn't get smarter at familiar routes. Diagnostic-first: dump cache hit rates / replay-vs-fresh-plan timings, look for a bug in the cache-key / cache-staleness logic before declaring it broken. Probably starts in `PathFinding/` (the cache layer over [BaritoneStylePathFinder](src/main/java/net/wcfcarolina13/PathFinding/BaritoneStylePathFinder.java) and classic [PathFinder](src/main/java/net/wcfcarolina13/PathFinding/PathFinder.java)).
