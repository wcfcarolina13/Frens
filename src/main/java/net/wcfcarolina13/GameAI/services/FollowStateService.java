...
public final class FollowStateService {
    ...
    public record VerticalClimbLock(BlockPos entryClimbPos,
                                    BlockPos entryStandPos,
                                    int topY,
                                    long expiresAtMs,
                                    int noProgressTicks,
                                    long lastProgressMs,
                                    BlockPos forcedDoorBase,
                                    int doorTraverseAttempts) {
    }
    ...
    public static final Map<UUID, BlockPos> FOLLOW_WAYPOINT_LAST_HEAD = new ConcurrentHashMap<>();
    public static final Map<UUID, BlockPos> FOLLOW_WAYPOINT_LAST_BOT_BLOCK = new ConcurrentHashMap<>();
    public static final Map<UUID, Integer> FOLLOW_WAYPOINT_HEAD_STAGNANT_TICKS = new ConcurrentHashMap<>();
    public static final Map<UUID, Integer> FOLLOW_WAYPOINT_REPOSITION_ATTEMPTS = new ConcurrentHashMap<>();
    public static final Map<UUID, Long> FOLLOW_WAYPOINT_REPOSITION_COOLDOWN_UNTIL_MS = new ConcurrentHashMap<>();
    public static final Map<UUID, BlockPos> FOLLOW_WAYPOINT_REPOSITION_ORIGIN_BLOCK = new ConcurrentHashMap<>();
    public static final Map<UUID, Long> FOLLOW_WAYPOINT_REPOSITION_VERIFY_UNTIL_TICK = new ConcurrentHashMap<>();
    public static final Map<UUID, Long> FOLLOW_LAST_FOLIAGE_LOG_MS = new ConcurrentHashMap<>();
    public static final Map<UUID, Long> FOLLOW_LAST_WAYPOINT_STUCK_LOG_MS = new ConcurrentHashMap<>();
    public static final Map<UUID, Long> FOLLOW_LAST_WAYPOINT_REPOSITION_LOG_MS = new ConcurrentHashMap<>();
    public static final Map<UUID, Long> FOLLOW_LAST_WAYPOINT_REACH_LOG_MS = new ConcurrentHashMap<>();
    public static final Map<UUID, BlockPos> FOLLOW_LAST_PLANNED_FIRST_WAYPOINT = new ConcurrentHashMap<>();
    public static final Map<UUID, BlockPos> FOLLOW_LAST_PLANNED_BOT_BLOCK = new ConcurrentHashMap<>();
    public static final Map<UUID, Integer> FOLLOW_REPEAT_FIRST_WAYPOINT_STREAK = new ConcurrentHashMap<>();
    public static final Map<UUID, Integer> FOLLOW_REPEAT_VERTICAL_TRAP_STREAK = new ConcurrentHashMap<>();
    public static final Map<UUID, Long> FOLLOW_LAST_REPEAT_WAYPOINT_LOG_MS = new ConcurrentHashMap<>();
    public static final Map<UUID, Long> FOLLOW_LAST_VERTICAL_TRAP_LOG_MS = new ConcurrentHashMap<>();
    public static final Map<UUID, Long> FOLLOW_LAST_TRANSITION_REJECT_LOG_MS = new ConcurrentHashMap<>();
    public static final Map<UUID, BlockPos> FOLLOW_WATER_ESCAPE_ORIGIN_BLOCK = new ConcurrentHashMap<>();
    public static final Map<UUID, Long> FOLLOW_WATER_ESCAPE_VERIFY_UNTIL_TICK = new ConcurrentHashMap<>();
    ...
    public static void setVerticalClimbLock(UUID botId, VerticalClimbLock lock) {
        if (botId == null) {
            return;
        }
        if (lock == null) {
        ...
        FOLLOW_VERTICAL_CLIMB_LOCK.put(botId, lock);
    }

    public static VerticalClimbLock getVerticalClimbLock(UUID botId) {
        if (botId == null) {
            return null;
        }
        return FOLLOW_VERTICAL_CLIMB_LOCK.get(botId);
    }

    public static void clearVerticalClimbLock(UUID botId) {
        if (botId == null) {
            return;
        }
        FOLLOW_VERTICAL_CLIMB_LOCK.remove(botId);
    ...
    public static void clearPlanning(UUID botId) {
        ...
        FOLLOW_LAST_FOLIAGE_LOG_MS.remove(botId);
        FOLLOW_LAST_WAYPOINT_STUCK_LOG_MS.remove(botId);
        FOLLOW_LAST_WAYPOINT_REPOSITION_LOG_MS.remove(botId);
        FOLLOW_LAST_WAYPOINT_REACH_LOG_MS.remove(botId);
        FOLLOW_LAST_PLANNED_FIRST_WAYPOINT.remove(botId);
        FOLLOW_LAST_PLANNED_BOT_BLOCK.remove(botId);
        FOLLOW_REPEAT_FIRST_WAYPOINT_STREAK.remove(botId);
        FOLLOW_REPEAT_VERTICAL_TRAP_STREAK.remove(botId);
        FOLLOW_LAST_REPEAT_WAYPOINT_LOG_MS.remove(botId);
        FOLLOW_LAST_VERTICAL_TRAP_LOG_MS.remove(botId);
        FOLLOW_LAST_TRANSITION_REJECT_LOG_MS.remove(botId);
    ...
    public static void reset() {
        ...
        FOLLOW_WAYPOINT_LAST_HEAD.clear();
        FOLLOW_WAYPOINT_LAST_BOT_BLOCK.clear();
        FOLLOW_WAYPOINT_HEAD_STAGNANT_TICKS.clear();
        FOLLOW_WAYPOINT_REPOSITION_ATTEMPTS.clear();
        FOLLOW_WAYPOINT_REPOSITION_COOLDOWN_UNTIL_MS.clear();
        FOLLOW_WAYPOINT_REPOSITION_ORIGIN_BLOCK.clear();
        FOLLOW_WAYPOINT_REPOSITION_VERIFY_UNTIL_TICK.clear();
        FOLLOW_LAST_FOLIAGE_LOG_MS.clear();
        FOLLOW_LAST_WAYPOINT_STUCK_LOG_MS.clear();
        FOLLOW_LAST_WAYPOINT_REPOSITION_LOG_MS.clear();
        FOLLOW_LAST_WAYPOINT_REACH_LOG_MS.clear();
        FOLLOW_LAST_PLANNED_FIRST_WAYPOINT.clear();
        FOLLOW_LAST_PLANNED_BOT_BLOCK.clear();
        FOLLOW_REPEAT_FIRST_WAYPOINT_STREAK.clear();
        FOLLOW_REPEAT_VERTICAL_TRAP_STREAK.clear();
        FOLLOW_LAST_REPEAT_WAYPOINT_LOG_MS.clear();
        FOLLOW_LAST_VERTICAL_TRAP_LOG_MS.clear();
        FOLLOW_LAST_TRANSITION_REJECT_LOG_MS.clear();
        FOLLOW_WATER_ESCAPE_ORIGIN_BLOCK.clear();
        FOLLOW_WATER_ESCAPE_VERIFY_UNTIL_TICK.clear();
    ...
    /**
     * Deterministic follow/come diagnostics snapshot for command assertions.
     */
    public static Map<String, Long> diagnosticSnapshot(UUID botId) {
        if (botId == null) {
            return Map.of();
        }
        Map<String, Long> out = new LinkedHashMap<>();
        out.put("repeat_wp_streak", (long) FOLLOW_REPEAT_FIRST_WAYPOINT_STREAK.getOrDefault(botId, 0));
        out.put("vertical_trap_streak", (long) FOLLOW_REPEAT_VERTICAL_TRAP_STREAK.getOrDefault(botId, 0));
        out.put("water_escape_attempts", (long) FOLLOW_WATER_ESCAPE_ATTEMPTS.getOrDefault(botId, 0));
        out.put("waypoint_reposition_attempts", (long) FOLLOW_WAYPOINT_REPOSITION_ATTEMPTS.getOrDefault(botId, 0));
        out.put("water_escape_verify_until_tick", FOLLOW_WATER_ESCAPE_VERIFY_UNTIL_TICK.getOrDefault(botId, 0L));
        out.put("water_escape_cooldown_until_ms", FOLLOW_WATER_ESCAPE_COOLDOWN_UNTIL_MS.getOrDefault(botId, 0L));
