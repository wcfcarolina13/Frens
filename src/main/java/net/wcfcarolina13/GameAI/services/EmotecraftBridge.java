...
public final class EmotecraftBridge {
    ...
    // Emotecraft 3.2.0 has no dedicated stopEmote API; setPlayerPlayingEmote(UUID, Animation)
    // accepts null as the "stop" signal. We resolve it once at init and invoke with null
    // when stopping; if the method ever disappears we fall back to a no-op stop.
    private static volatile Method SET_EMOTE_METHOD;     // ServerEmoteAPI.setPlayerPlayingEmote(UUID, Animation)
    ...
    public static void initialize() {
        ...
        try {
            ...
            // Emotecraft 3.2.0 has no dedicated stopEmote — setPlayerPlayingEmote(UUID, null)
            // is the supported way to clear an active emote. Optional: if the method ever
            // disappears we still ship, looping dances just won't stop early.
            Method setEmote = null;
            try {
                setEmote = api.getMethod("setPlayerPlayingEmote", UUID.class, animationClass);
            } catch (NoSuchMethodException ignored) {
                LOGGER.warn("Emotecraft.setPlayerPlayingEmote(UUID, Animation) not found; long-running emotes will not be cancelable");
            }

            GET_EMOTE_METHOD = getEmote;
            PLAY_EMOTE_METHOD = playEmote;
            SET_EMOTE_METHOD = setEmote;
            AVAILABLE = true;
            LOGGER.info("Emotecraft detected; bot emote bridge enabled ({} emotes registered, stopEmote={})",
                    EmoteId.values().length, setEmote != null ? "yes" : "no");
        } catch (Throwable t) {
    ...
    public static boolean stopEmote(ServerPlayerEntity bot) {
        if (!AVAILABLE || bot == null || SET_EMOTE_METHOD == null) return false;
        try {
            // setPlayerPlayingEmote(UUID, Animation) with null Animation = stop
            SET_EMOTE_METHOD.invoke(null, bot.getUuid(), null);
            return true;
        } catch (Throwable t) {
