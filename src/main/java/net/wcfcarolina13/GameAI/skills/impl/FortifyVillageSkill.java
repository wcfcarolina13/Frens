 ...
 */
public final class FortifyVillageSkill implements Skill, FortifySkillOps.FortifyNavOps, FortifySkillOps.FortifyTowerContext, FortifySkillOps.FortifyCarveContext {
    ...
    // Field order matters: cleanupProcessor, towerHelper and carveHelper below receive the helpers declared above them.
    ...
    /** Tower construction/scaffold/approach/hard-reset logic (extracted from this skill). */
    private final FortifyTowerHelper towerHelper = new FortifyTowerHelper(this, this, entombmentHelper, cleanupHelper);
    /** Carve / break-through navigation logic (extracted from this skill). */
    private final FortifyCarveHelper carveHelper =
            new FortifyCarveHelper(this, this, entombmentHelper, cleanupHelper, cleanupProcessor,
                    () -> fortificationProtectedPositions);
    ...
    private SkillExecutionResult handlePatch(ServerCommandSource source, ServerPlayerEntity bot,
        ...
        do {
            ...
            while (!remainingEdges.isEmpty()) {
                ...
                if (edgeIdx == -1) {
                    int towerRepaired = towerHelper.patchTowerBlocks(
                            source,
                            bot,
                            world,
                            entry.getValue(),
    ...
    private SkillExecutionResult buildWall(ServerCommandSource source, ServerPlayerEntity bot,
        ...
        if (!completedEdges.contains(-1) && !resumedRun) {
            ...
            if (!towerBlocks.isEmpty()) {
                int towerPlaced = 0;
                List<WallPoint> towerVertices = towerHelper.orderAndDedupeTowerVertices(layout.hullVertices(), bot.getBlockPos());
                Map<Integer, List<ProceduralWallBlock>> blocksByVertex =
                        towerHelper.groupTowerBlocksByNearestVertex(towerBlocks, towerVertices);
                ...
                for (int vi = 0; vi < towerVertices.size(); vi++) {
                    ...
                    if (towerHelper.isTowerComplete(presentCount, plannedCount)) {
                        continue;
                    }
                    ...
                    int placed = towerHelper.executeTowerVertexWithRetries(
                            source,
                            bot,
                            world,
                            vertex,
                    ...
                    if (!towerHelper.isTowerComplete(presentAfter, plannedCount)) {
                        ChatUtils.sendSystemMessage(source, String.format(
    ...
    private boolean tryBreakThroughObstacle(ServerPlayerEntity bot, ServerWorld world, BlockPos target, String causeContext) {
        return carveHelper.tryBreakThroughObstacle(bot, world, target, causeContext);
    }
    ...
    private boolean isCarveEligibleForBreakAttempt(FortifyNavRuntimeScope scope, ServerWorld world, BlockPos botPos, BlockPos target) {
        return carveHelper.isCarveEligibleForBreakAttempt(scope, world, botPos, target);
    }
    ...
    @Override
    public boolean hasCurrentFortificationLayout() {
    ...
    @Override
    public boolean isInsideCurrentFortificationHull(BlockPos pos) {
    ...
    @Override
    public boolean isLayoutExteriorReachable(ServerWorld world, BlockPos pos) {
    ...
    private void attemptFinalizeCarveTransaction(ServerPlayerEntity bot, ServerWorld world, FortifyNavRuntimeScope scope) {
        carveHelper.attemptFinalizeCarveTransaction(bot, world, scope);
    }
    @Override
