...
import net.wcfcarolina13.GameAI.skills.DirtNavigationPolicy;
...
public final class DirtShovelSkill implements Skill {
    ...
    public SkillExecutionResult execute(SkillContext context) {
        ...
        if (!resumeRequested) {
            net.wcfcarolina13.GameAI.skills.support.MiningHazardDetector.clear(player);
        }
        ...
        try {
            ...
            for (BlockPos detectedPos : candidates) {
                ...
                detection.adjacentWarnings().forEach(adjacent ->
                        ChatUtils.sendChatMessages(source.withSilent().withPermissions(net.wcfcarolina13.Frens.OPERATOR_PERMISSIONS), adjacent.chatMessage()));
                if (detection.blockingHazard().isPresent()) {
                    ...
                    ChatUtils.sendChatMessages(source.withSilent().withPermissions(net.wcfcarolina13.Frens.OPERATOR_PERMISSIONS), hazard.chatMessage());
