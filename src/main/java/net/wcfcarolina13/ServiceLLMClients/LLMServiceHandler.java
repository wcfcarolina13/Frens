package net.wcfcarolina13.ServiceLLMClients;
...
import net.wcfcarolina13.ChatUtils.Helper.RAG2;
import net.wcfcarolina13.ChatUtils.NLPProcessor;
import net.wcfcarolina13.Exception.intentMisclassification;
...
public class LLMServiceHandler {
        ...
        CompletableFuture<String> initFuture = CompletableFuture.supplyAsync(() -> {
            try {
                if (client.isReachable()) {
                    ...
                    ChatUtils.sendChatMessages(botSource, "Established connection to " + client.getProvider() + "'s servers. Using " + Frens.CONFIG.getSelectedLanguageModel());
    ...
    private static void routeIntent(String message, ServerCommandSource botSource, UUID playerUUID, LLMClient client) throws Exception {
        ...
        if (botEntity != null && isStatusRequest(message)) {
            net.wcfcarolina13.GameAI.llm.LLMStatusReporter.respond(botSource, playerUUID);
            return;
        }
    ...
    public static void runQueuedCommand(ServerCommandSource botSource, UUID commanderId, String message) {
        ...
        server.execute(() -> {
            try {
                String provider = System.getProperty("frens.llmMode", System.getProperty("aiplayer.llmMode", "ollama"));
