 ...
 * Follows the same pattern as {@link net.shasankp000.GameAI.services.BotHomeService}:
 * Gson, per-world keyed, synchronized access, immediate flush on writes.
 *
 * <p>Storage file: {@code config/ai-player/bot_fortifications.json}
 */
public final class FortificationPersistenceService {
    ...
    /**
     * Mark a specific edge as completed and update progress.
     */
    public static void markEdgeComplete(MinecraftServer server, String worldKey, String name,
                                         int edgeIndex, int blocksPlacedThisEdge) {
        ensureLoaded();
        synchronized (LOCK) {
            WorldData wd = DATA.worlds.get(worldKey);
            if (wd == null) return;
