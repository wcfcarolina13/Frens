...
public final class SurvivalRecruitmentService {
    ...
    public static ManualConfig.SurvivalRecruitmentState getState(MinecraftServer server) {
        ...
        return AIPlayer.CONFIG.getOrCreateSurvivalRecruitmentState(worldKey(server));
    }
    ...
    public static void handleConfirmRecruit(ServerPlayerEntity player, String botAlias) {
        ...
        try {
            AIPlayer.LOGGER.info("Recruit confirm invoked by {} requestedAlias='{}' dim={} pos={}",
        ...
        try {
            AIPlayer.LOGGER.info("Recruit confirm eligibility: eligibleNow={} grantedRecently={} grantedAgeTicks={}",
                    eligibleNow,
                    grantedRecently,
        ...
        if (existing != null && !existing.isRemoved() && existing.isAlive()) {
            ...
            AIPlayer.LOGGER.info("Recruitment spawning bot alias={} dim={} at {}", alias, dim.getValue(), spawn);
        ...
        if (AIPlayer.CONFIG != null) {
            AIPlayer.CONFIG.ensureOwner(alias, player.getUuid(), player.getName().getString());
            ManualConfig.SurvivalRecruitmentState updated = AIPlayer.CONFIG.getOrCreateSurvivalRecruitmentState(worldKey(server));
    ...
    public static void handleRequestReplayRecruitment(ServerPlayerEntity player, String requestedAlias) {
        ...
        ManualConfig.SurvivalRecruitmentState updated = AIPlayer.CONFIG.getOrCreateSurvivalRecruitmentState(worldKey);
        ...
        for (ServerPlayerEntity p : server.getPlayerManager().getPlayerList()) {
            ...
            ServerPlayNetworking.send(p, new net.shasankp000.network.CompanionQuestStatePayload(alias, 0, false));
        }
    ...
    public static void noteCompanionDeath(MinecraftServer server, ServerPlayerEntity bot) {
        ...
        if (!isEnabled(server) || AIPlayer.CONFIG == null) {
            return;
        }
